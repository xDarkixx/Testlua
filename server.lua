...
local function saveConfig()
 ...
 f:write(serialization.serialize({sensor=AUTO_SENSOR,tempMin=TEMP_MIN,tempMax=TEMP_MAX,energyMin=ENERGY_MIN,energyMax=ENERGY_MAX,rodMin=ROD_MIN,rodMax=ROD_MAX,rodStep=ROD_STEP}))
 f:close()
end
...
local function loadConfig()
 ...
 local d=f:read(math.huge); f:close()
 local ok,t=pcall(serialization.unserialize,d)
 if ok and type(t)=="table" then
  AUTO_SENSOR=t.sensor or AUTO_SENSOR
  TEMP_MIN=tonumber(t.tempMin) or TEMP_MIN
  TEMP_MAX=tonumber(t.tempMax) or TEMP_MAX
  ENERGY_MIN=tonumber(t.energyMin) or ENERGY_MIN
  ENERGY_MAX=tonumber(t.energyMax) or ENERGY_MAX
  ROD_MIN=tonumber(t.rodMin) or ROD_MIN
  ROD_MAX=tonumber(t.rodMax) or ROD_MAX
  ROD_STEP=tonumber(t.rodStep) or ROD_STEP
 end
end
loadConfig()
...
local function findStargate()
 ...
 if list then
  local addr=list()
  if addr then
   local ok2,p2=pcall(component.proxy,addr)
...
local function sgcall(method,...)
 local p=ensureSG()
 if not p or type(p[method])~="function" then return false,nil end
 local ok,a,b,c=pcall(p[method],...)
 if not ok then setLog("SG FEHLER: "..tostring(a)); return false,nil end
 return true,{a,b,c}
end

local function refreshSG()
 if not ensureSG() then
  sg.state="Offline"; sg.engaged=0; sg.iris="N/A"; sg.address="N/A"; return
 end
 local ok,r=sgcall("stargateState")
 if ok and r then
  sg.state=tostring(r[1] or "Unknown")
  sg.engaged=tonumber(r[2] or 0) or 0
  sg.direction=tostring(r[3] or "")
 end
 local ok2,r2=sgcall("irisState")
 if ok2 and r2 then sg.iris=tostring(r2[1] or "UNKNOWN") end
 local ok3,r3=sgcall("localAddress")
 if ok3 and r3 then sg.address=tostring(r3[1] or "N/A") end
end

local function sgAction(method,arg)
 local p=ensureSG()
 if not p or type(p[method])~="function" then
  setLog("SG: "..method.." NICHT VERFÜGBAR")
  return false
 end
 local ok,err
...
local function dialGate(addr)
 ...
 if addr=="" then setLog("SG: KEINE ADRESSE"); return end
 if sgAction("dial",addr) then setLog("SG: WÄHLE "..addr) end
end

local function iris(open)
 if sgAction(open and "openIris" or "closeIris") then
  setLog(open and "IRIS GEÖFFNET" or "IRIS GESCHLOSSEN")
 end
end

local function disconnectGate()
 if sgAction("disconnect") then setLog("SG: VERBINDUNG GETRENNT") end
end
...
local function averageRods()
 if type(reactor.rodLevels)=="table" and reactor.rodCount>0 then
  local sum,count=0,0
  for i=0,reactor.rodCount-1 do
   local v=tonumber(reactor.rodLevels[i])
...
local function makeRodLevels(level)
 level=math.max(ROD_MIN,math.min(ROD_MAX,tonumber(level) or 0))
 local levels={}
 local count=math.max(1,tonumber(reactor.rodCount) or 1)
...
local function setRods(level)
 level=math.max(ROD_MIN,math.min(ROD_MAX,tonumber(level) or 0))
 reactor.rods=level
 reactor.rodLevels=makeRodLevels(level)
 sendReactor("RODS",level)
end

local function changeRods(delta)
 setRods(averageRods()+delta)
 setLog(string.format("STÄBE: %.0f %%",reactor.rods))
end
...
local function autoControl()
 ...
 if AUTO_SENSOR=="HYBRID" then
  if reactor.temperature>=TEMP_MAX or reactor.energy>=ENERGY_MAX then
   setRods(ROD_MAX); sendReactor("AUS",true); setLog("AUTO: SICHERHEITSABSCHALTUNG")
  elseif reactor.temperature<=TEMP_MIN and reactor.energy<=ENERGY_MIN then
   setRods(ROD_MIN); sendReactor("AN",true); setLog("AUTO: REAKTOR START")
  end
  return
 end
...
local function makeStatus()
 return {
  ...
  rods=reactor.rods,
  gesamtRF=reactor.rfProTick,
  lastDaten={
   tempKern=reactor.temperature,
   rfProTick=reactor.rfProTick,
   prozent=reactor.energy,
   steuerstaebe=reactor.rods,
   fuelAmt=reactor.fuelAmt,
   wasteAmt=reactor.wasteAmt,
   rodLevels=reactor.rodLevels
  },
  reactorOnline=reactor.online,
  reactorActive=reactor.active,
  sgState=sg.state,
  sgChevrons=sg.engaged,
  sgIris=sg.iris,
  sgAddress=sg.address,
...
local function sendStatus(to)
 if not to then return end
 pcall(modem.send,to,PORT_SERVER,serialization.serialize(makeStatus()))
end
...
local function refreshGUI()
 ...
 line(7,string.format("Temperatur:  %.1f C",reactor.temperature or 0))
 line(8,string.format("Energy:      %.1f %%",reactor.energy or 0))
 line(9,string.format("Stäbe:       %.0f %%",reactor.rods or 0))
...
local function touch(x,y)
 ...
 elseif y==19 then
  if x<14 then modus="AUTO"; setLog("REAKTOR: AUTO")
  elseif x<29 then modus="MANUELL_AN"; sendReactor("AN",true); setLog("REAKTOR: START")
  elseif x<48 then modus="MANUELL_AUS"; sendReactor("AUS",true); setLog("REAKTOR: STOPP") end
 elseif y==21 then
  if x<15 then shell.execute("floppy_backup.lua","backup") elseif x<30 then shell.execute("floppy_backup.lua","restore") else shell.execute("floppy_backup.lua","status") end
 end
 refreshGUI()
end
...
local function packet(localAddress,senderAddress,port,distance,message)
 ...
 if port==PORT_REACTOR then
  ...
  reactor.active=message.active==true or message.active==1 or message.istAktiv==true
  ...
  reactor.rfProTick=tonumber(message.rfProTick or 0) or 0
  reactor.fuelAmt=tonumber(message.fuelAmt or 0) or 0
  reactor.wasteAmt=tonumber(message.wasteAmt or 0) or 0
 ...
 elseif port==PORT_SERVER then
  if message.cmd=="GET_DATA" then
   sendStatus(senderAddress)
  elseif message.cmd=="SG_DIAL" then
   dialGate(message.address or message.val)
   sendStatus(senderAddress)
  elseif message.cmd=="SG_IRIS_OPEN" then
   iris(true); sendStatus(senderAddress)
  elseif message.cmd=="SG_IRIS_CLOSE" then
   iris(false); sendStatus(senderAddress)
  elseif message.cmd=="SG_DISCONNECT" then
   disconnectGate(); sendStatus(senderAddress)
  elseif message.cmd=="RODS_UP" then
   changeRods(-ROD_STEP); sendStatus(senderAddress)
  elseif message.cmd=="RODS_DOWN" then
   changeRods(ROD_STEP); sendStatus(senderAddress)
  elseif message.cmd=="SET_MODUS" then
   local requested=tostring(message.val or "AUTO")
   if requested=="AUTO" or requested=="MANUELL_AN" or requested=="MANUELL_AUS" then modus=requested end
   if modus=="MANUELL_AN" then sendReactor("AN",true) elseif modus=="MANUELL_AUS" then sendReactor("AUS",true) end
   setLog("MODUS: "..modus)
   sendStatus(senderAddress)
  elseif message.cmd=="AUTO" then
   modus="AUTO"; setLog("REAKTOR: AUTO"); sendStatus(senderAddress)
  elseif message.cmd=="AN" then
   modus="MANUELL_AN"; sendReactor("AN",true); setLog("REAKTOR: START"); sendStatus(senderAddress)
  elseif message.cmd=="AUS" then
   modus="MANUELL_AUS"; sendReactor("AUS",true); setLog("REAKTOR: STOPP"); sendStatus(senderAddress)
  end
 end
end
...
while true do
 ...
 if ev=="modem_message" then
  -- OpenComputers: event, localAddress, remoteAddress, port, distance, message
  packet(a,b,c,d,e)
 elseif ev=="touch" then
  -- OpenComputers: touch, screenAddress, x, y, button, user
  touch(tonumber(b) or 0,tonumber(c) or 0)
 end
