...
local function playChevronSound(isFinal)
  pcall(function()
    computer.beep(200,0.04); os.sleep(0.02); computer.beep(150,0.06)
    if isFinal then computer.beep(520,0.25) else computer.beep(400,0.12) end
  end)
end
...
local function drawSGCGateSystem(x,y,state,chevronsEngaged)
  ...
  gpu.setBackground(C_PANEL); gpu.setForeground(C_TEXT_MUTED); gpu.set(x+2,y+13,"SGC Passierende Glyphe:")
  local glyphenIndex=(math.floor(ringWinkel/(360/39))%39)+1
  gpu.setForeground(C_CYAN); gpu.set(x+25,y+13,string.format("[%02d] %s",glyphenIndex,GLYPH_CODES[glyphenIndex] or "UNKNOWN"))
end
